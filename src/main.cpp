...
bool tone50hz = false;
int counter50hz = 0;
bool tone38khz = false;
int counter38khz = 0;

Servo myservoopenDoor;
Servo myservocloseDoor;
...
void IRAM_ATTR onTimer() // compteur de 500ms
{
  ...
  if (tone38khz)
  {
    digitalWrite(PIN_IN_PROX, !digitalRead(PIN_IN_PROX));
  }
  if (tone50hz)
  {
    if (counter38khz % 760 == 0)
    {
      digitalWrite(PIN_IR_TX, !digitalRead(PIN_IR_TX));
      counter38khz = 0;
    }
    counter38khz++;
  }
...
void distribute()
{
  ...
  while (digitalRead(PIN_SWITCH_DIST) == LOW)
  {
  } // tourne
  while (digitalRead(PIN_SWITCH_DIST) == HIGH)
  {
  }                                  // tourne encore
...
void openDoor()
{
  myservoopenDoor.setPeriodHertz(50); // standard 50 hz servo
  // ESP32PWM::allocateTimer(1);
  if (!doorOpened)
  {
    myservoopenDoor.attach(PIN_SERVO);
    for (int pos = 169; pos >= 90; pos -= 1)
    {
      myservoopenDoor.write(pos);
      delay(20);
    }
    myservoopenDoor.detach();
    doorOpened = true;
  }
}
...
void closeDoor()
{
  myservocloseDoor.setPeriodHertz(50); // standard 50 hz servo
  // ESP32PWM::allocateTimer(1);
  if (doorOpened)
  {
    myservocloseDoor.attach(PIN_SERVO);
    for (int pos = 90; pos <= 169; pos += 1)
    {
      // in steps of 1 degree
      myservocloseDoor.write(pos);
      delay(20);
    }
    myservocloseDoor.detach();
    doorOpened = false;
  }
}
...
void setup()
{
  ...
  int nbTries = 0;
  while (WiFi.status() != WL_CONNECTED && nbTries < 10)
  { // timeout
    delay(500);
    Serial.print(".");
    nbTries++;
  ...
  unsigned int timerFactor = 26;               // 500ms
...
void loop()
{
  ...
  /*if (IrReceiver.decode()) {
    ...
    Serial.println();
    if (IrReceiver.decodedIRData.address == 0x12) {
  ...
  if (!doorOpened || stateComm == CAT_EATING)
  {
    ...
    case TX_COMM:
    {
      if (counterTXUptime >= 1)
      {                        // 500ms se sont passé
        setRGB(100, 100, 100); // blanc
        stateComm = RX_COMM;
        counterRXUptime = 0;
        // noTone(PIN_IR_TX); //ferme DEL IR
        tone50hz = false;
        IrReceiver.begin(PIN_IR_RX); // Initializes the IR receiver object
        IrReceiver.enableIRIn(); // reconfigure timer pour décodage infarouge
        IrReceiver.resume();     // permet la lecture décodeur infrarougeù
      }
      break;
    }
    case RX_COMM:
    {
      if (counterRXUptime >= 2)
      {                  // timeout 2 seconde
        setRGB(0, 0, 0); // ferme RGB
        stateComm = STANDBY;
        IrReceiver.stop();
      ...
      if (IrReceiver.decode())
      { // décode IR recu de la balise
        Serial.println("Received something...");
        IrReceiver.printIRResultShort(&Serial); // imprime donnée IR recu
        Serial.println();
        if ((IrReceiver.decodedIRData.protocol == NEC || IrReceiver.decodedIRData.protocol == NEC2) && IrReceiver.decodedIRData.address == 0x12)
        { // check le protocole et adresse envoyé
    ...
    case CAT_EATING:
    {
      ...
      Serial.println(prox);
      if (counterDoorOpenCatLeft >= 3)
      { // chat parti depuis 3 seconde
        closeDoor();
        setRGB(0, 0, 0);
  ...
  case PORTIONS_MENU:
    ...
    if (keypad_right)
    {
      ...
      else if (Item_selected_row == 1 && Item_selected_column == 2)
      { // vers bouton retour
        Item_selected_row = 3;
        Item_selected_column = 1;
        printPortionsMenuNavigation();
    ...
    if (keypad_select)
    {
      if (Item_selected_row == 3 && Item_selected_column == 1)
      { // retour menu navigation
        menu_selected = NAVIGATION_MENU;
        printMenu(NAVIGATION_MENU);
        Item_selected_column = 1;
        Item_selected_row = 1;
      ...
      if (Item_selected_row == 2)
      { // passe de chiffre en chiffre dans l'heure
  ...
  case ACTIONS_MENU:
    ...
    if (keypad_up)
    {
      ...
      {
        Item_selected_row--;
        if (Item_selected_row == 2 || Item_selected_row == 4)
        { // remettre colomne a 1
          Item_selected_column = 1;
        }
    ...
    if (keypad_left)
    {
      ...
      if (Item_selected_column == 1 && Item_selected_row == 4)
      { // retour sur ligne 3 et ligne 4
        Item_selected_column = 2;
        Item_selected_row = 3;
        printActionsMenuButtons();
      }
    ...
    if (keypad_right)
    {
      ...
      else if (Item_selected_column == 2 && Item_selected_row == 3)
      { // retour sur ligne 3 et ligne 4
        Item_selected_column = 1;
        Item_selected_row = 4;
        printActionsMenuButtons();
    ...
    if (keypad_select)
    {
      ...
      if (Item_selected_row == 3 && Item_selected_column == 2)
      { // reset balise
      }
      if (Item_selected_row == 4 && Item_selected_column == 1)
      { // retour menu navigation
        menu_selected = NAVIGATION_MENU;
        printMenu(NAVIGATION_MENU);
        Item_selected_column = 1;
